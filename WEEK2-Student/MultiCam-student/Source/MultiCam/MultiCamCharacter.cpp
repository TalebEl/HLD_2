...
AMultiCamCharacter::AMultiCamCharacter()
{
	...
	MoveComponent->JumpZVelocity = 620;
	//SET MaxWalkSpeedCrouched to 200
	MoveComponent->MaxWalkSpeedCrouched = 200;
	//SET bOrientRotationToMovement to true
	MoveComponent->bOrientRotationToMovement = true;					 //comment - Character moves in the direction of input...
	//SET RotateRate on the movement component to FRotator(0.0f, 540.0f, 0.0f)
	MoveComponent->RotationRate = FRotator(0.0f, 540.0f, 0.0f);			 //comment - ...at this rotation rate
	//SET the aircontrol on the movement component to 0.2f
	MoveComponent->AirControl = 0.2f;

	//comment - Create a camera boom (pulls in towards the player if there is a collision)
	...
	CameraBoom->bUsePawnControlRotation = true;	//comment - Rotate the arm based on the controller

	//comment - Create a follow camera
	//CREATE the FollowCamera
	FollowCamera = CreateDefaultSubobject<UCameraComponent>("FollowCamera");
	//ATTAch The FollowCamera to the CameraBoom at USpringArmComponent::SocketName
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName);				 //comment - Attach the camera to the end of the boom and let the boom adjust to match the controller orientation
	//SET bUsePawnControlRotation on the FollowCamera to false
	FollowCamera->bUsePawnControlRotation = false;										 //comment - Camera does not rotate relative to arm
	
	//comment - First person camera and arms
	//
	...
	FirstPersonCamera->SetRelativeLocation(FVector(-39.56f, 1.75f, 64.f));										 //comment - Position the camera
	...
	OverShoulderCameraBoom = CreateDefaultSubobject<USpringArmComponent>("OverShoulderCameraBoom");
	...
	OverShoulderCameraBoom->bUsePawnControlRotation = false;												//comment -  Fixed Camera
	...
	OverShoulderCamera = CreateDefaultSubobject<UCameraComponent>("OverShoulderCamera");
	//ATTACH the OverShoulderCamera to the OverShoulderCameraBoom at USpringArmComponent::SocketName
	OverShoulderCamera->SetupAttachment(OverShoulderCameraBoom, USpringArmComponent::SocketName);												 //comment -  Attach the camera to the end of the boom and let the boom adjust to match the controller orientation
	//SET OverShoulderCamera->bUsePawnControlRotation to false
	OverShoulderCamera->bUsePawnControlRotation = false;
	
	//GET the Mesh from ACharacter (This is the main mesh)
	...
	CameraZoomMaximumDistance = 600.0f;
	//SET CameraZoomMinimumDistance to 100.f
	CameraZoomMinimumDistance = 100.0f;
	//SET CameraZoomCurrent to 300.f
	CameraZoomCurrent = 300.0f;
	//SET CameraZoomIncrement to 20.f
	CameraZoomIncrement = 20.0f;
...
void AMultiCamCharacter::UpdateForCameraMode()
{
	...
	//GET the PlayerController and Assign it to a variable called OurPlayerController
...
void AMultiCamCharacter::SetThirdPersonSmoothFollow()
{
	...
	//CALL GetMesh()->MarkRenderStateDirty()
	GetMesh()->MarkRenderStateDirty();
	//CALL SetVisibleFlag() on ArmsMesh passing in false
	ArmsMesh->SetVisibleFlag(false); //bVisible = false;
	//CALL MarkRenderStateDirty() on ArmsMesh
	ArmsMesh->MarkRenderStateDirty();
	//CALL SetVisibleFlag() on BodyMesh passing in false
	BodyMesh->SetVisibleFlag(false); //bVisible = false;
...
void AMultiCamCharacter::SetThirdPersonOverShoulder()
{
	//CALL GetMesh()->SetOnlyOwnerSee() passing in false
	GetMesh()->SetOnlyOwnerSee(false);
	//CALL GetMesh()->MarkRenderStateDirty()
...
void AMultiCamCharacter::SetFirstPerson()
{
	//CALL GetMesh()->SetOnlyOwnerSee() passing in false
	GetMesh()->SetOnlyOwnerSee(false);
	//CALL GetMesh()->MarkRenderStateDirty()
	...
	//GetMesh()->SetOnlyOwnerSee(true); //DOES NOT WORK Play around with it
	...
	//SET bIsResetting to false
	bIsResetting = false;
	//CALL SetVisibleFlag() on ArmsMesh passing in true
	ArmsMesh->SetVisibleFlag(true); //bVisible = true;
	//CALL MarkRenderStateDirty() on ArmsMesh
	ArmsMesh->MarkRenderStateDirty();
	//CALL SetVisibleFlag() on BodyMesh passing in true
	BodyMesh->SetVisibleFlag(true); //bVisible = true;
...
void AMultiCamCharacter::SetCamera()
{
	...
	if (bThirdPersonCam)//(CameraModeEnum == ECharacterCameraMode::ThirdPersonDefault || CameraModeEnum == ECharacterCameraMode::ThirdPersonReseting || CameraModeEnum == ECharacterCameraMode::ThirdPersonOverShoulder)
	{
		...
		GetMesh()->MarkRenderStateDirty();
		//CALL SetVisibleFlag() on ArmsMesh passing in false
		ArmsMesh->SetVisibleFlag(false); //SetVisibility(false);
		//CALL MarkRenderStateDirty() on ArmsMesh
		ArmsMesh->MarkRenderStateDirty();
		//CALL SetVisibleFlag() on BodyMesh passing in false
		BodyMesh->SetVisibleFlag(false); //bVisible = false;
	...
	else 
	{
		...
		bUseControllerRotationYaw = true;
		//SET GetMesh()->bOwnerNoSee to true
		GetMesh()->bOwnerNoSee = true;						 //comment - COMMENT TO SEE WHAT HAPPENS
		//CALL GetMesh()->MarkRenderStateDirty()
		GetMesh()->MarkRenderStateDirty();
		//CALL SetVisibleFlag() on ArmsMesh passing in true
		ArmsMesh->SetVisibleFlag(true);
		//CALL MarkRenderStateDirty() on ArmsMesh
		ArmsMesh->MarkRenderStateDirty();
		//CALL SetVisibleFlag() on BodyMesh passing in true
		BodyMesh->SetVisibleFlag(true);
		//CALL SetOnlyOwnerSee() on BodyMesh passing in true
		BodyMesh->SetOnlyOwnerSee(true);
		//CALL MarkRenderStateDirty() on BodyMesh
		BodyMesh->MarkRenderStateDirty();
